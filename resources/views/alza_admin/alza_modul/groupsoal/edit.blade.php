        ...
        <div class="card">
            ...
            <div class="card-content">
                <div class="card-body">
                    ...
                    @endif
                    {!! Form::model($groupsoal, ['method' => 'PATCH', 'route' => [config('pathadmin.admin_prefix') . 'groupsoals.update', $groupsoal->id], 'enctype' => 'multipart/form-data']) !!}
                    <div class="form-body">
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group"><label>Entitas</label>
                                    <select class="form-control" name="entitas_id">
                                        @foreach ($entitas as $result)
                                            <option value="{{ $result->id }}" {!! $groupsoal->entitas_id == $result->id ? 'selected' : '' !!}>
                            ...
                            <div class="col-12">
                                <div class="form-group"><label>Mapel</label><select class="form-control"
                                        ...
                                        @foreach ($mapel as $result)
                                            <option value="{{ $result->id }}" {!! $groupsoal->mapel_id == $result->id ? 'selected' : '' !!}>
                                                {{ $result->nama_mapel }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="form-group"><label>Pengajar</label><select class="form-control"
                                        ...
                                        @foreach ($pengajar as $result)
                                            <option value="{{ $result->id }}" {!! $groupsoal->pengajar_id == $result->id ? 'selected' : '' !!}>
                            ...
                            <div class="col-12">
                                <div class="form-group"><label>Catatan</label>
                                    <textarea name="catatan" id="catatan" cols="30" rows="10" class="form-control">{!! htmlspecialchars_decode(html_entity_decode($groupsoal->catatan)) !!}</textarea>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group"><label>waktu</label><input name="waktu" class="form-control"
                                        type="number" value="{{ $groupsoal->waktu }}"></div>
                            </div>
                            <div class="col-12">
                                <div class="form-group"><label>start_date</label><input name="start_date"
                                        class="form-control" type="date" value="{{ $groupsoal->start_date }}"></div>
                            </div>
                            <div class="col-12">
                                <div class="form-group"><label>expired_date</label><input name="expired_date"
                                        class="form-control" type="date" value="{{ $groupsoal->expired_date }}"></div>
                            </div>
                            <div class="col-12 d-flex justify-content-end border-top">
                                ...
                                <a class="btn btn-light-secondary btn-sm mr-1 mb-1 mt-1"
                                    href="{{ route(config('pathadmin.admin_prefix') . 'groupsoals.index') }}"> Batal</a>
                            </div>
                        </div>
                    </div>
                    {!! Form::close() !!}
